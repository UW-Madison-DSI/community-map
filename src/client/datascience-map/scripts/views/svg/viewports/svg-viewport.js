...
export default BaseView.extend({
	...
	toPoint: function(h, v) {
		let x = (h - this.width / 2) / this.scale;
		let y = (v - this.height / 2) / this.scale;		
		x -= this.offset.x;
		y -= this.offset.y;
	...
	setGrid: function(grid) {
		...
		// render grid
		//
		let backgroundLayer = $(this.el).find('#background.layer')[0];
		if (backgroundLayer) {
	...
	setScale: function(element, scale) {
		...
		//
			...
			let translate1 = 'translate(' + (x) + ',' + (y) + ')';
			let translate2 = 'translate(' + (-x) + ',' + (-y) + ')';
			scale = 'scale(' + 1 / scale + ')';
