...
export default FormView.extend({
	...
	template: _.template(`
		...
		<fieldset>
				...
				<div class="controls">
					<div class="checkbox">
						<label>
							...
							Have you applied and been selected to participate in the <a href="https://dsi.wisc.edu/about/dsi-affiliates" target="_blank">Data Science Institute Affiliate Program</a>?
						</label>
					</div>
				</div>
			</div>
			...
			<div class="form-group" id="research-interests">
				<label class="control-label">Research Interests</label>
			...
			<div class="form-group" id="other-interests">
				<label class="control-label">Other Research Interests</label>
				<div class="controls">
					<div class="input-group">
						<textarea rows="5"></textarea>
						<div class="input-group-addon">
							<i class="active fa fa-question-circle" data-toggle="popover" title="Other Research Interests" data-content="This is a comma separated list of items of interest to you."></i>
						</div>	
					</div>
				</div>
			</div>
	...
	getOtherTerms: function() {
		let values = [];
		let value = this.$el.find('#other-interests textarea').val().trim();
	...
	setValue: function(key, value) {
		switch (key) {
			...
			case 'research_terms':
				this.setTerms(value);
				this.$el.find('#other-interests textarea').val(this.getOtherAttributes(value).join(', '));
				break;
	...
	showTermSelectors: function() {
		...
		for (let i = 0; i < keys.length; i++) {
			...
			this.showChildView(id, new TreeView({
				...
				expanded: defaults.sidebar.interests? defaults.sidebar.interests.expanded : undefined
			}));
		}
	},
	...
	onClickCommunities: function() {
		...
		this.$el.find('#other-interests textarea').val(this.getOtherAttributes(terms).join(', '));
		this.update();
	},
