...
export default BaseView.extend({
	...
	showUserValidationErrorDialog: function(options) {
		import(
		...
		if (this.getChildView('form').isValid()) {
			...
			let response = this.model.checkValidation(this.model.toJSON(), {
						...
						success: () => {
							...
							if (this.model.has('email')) {
								...
								application.showNotifyDialog({
									"message": "Your account was successfully created.  You may now sign in by clicking the Sign In button to create your profile."
								});
							}
						},
				...
				error: () => {
					...
					//
					this.showUserValidationErrorDialog({
						errors: errors
					});
				}
			});
